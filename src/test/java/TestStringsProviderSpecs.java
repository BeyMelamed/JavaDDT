 ...
 * ==========|===============|========================================================
 */
public class TestStringsProviderSpecs {
   ...
   private void determineErrors() {
      if (getArgs() == null) {
         setErrors("Blank Input Specs.");
         return;
      }

      if (getArgs().length < 2) {
         setErrors("Input Specs string is not properly constructed - should have at least Type and Input Source delimited by '" + SPLITTER + "'");
         return;
      }

      if (isFileProvider() && isBlank(getFileName())) {
         setErrors("Source File Name is required but is blank.");
         return;
      }

      if (isInlineProvider() && isBlank(getClassName())) {
         setErrors("Source Class Name is required but is blank.");
         return;
      }

      if (isInlineProvider() && isBlank(getItemsContainerName())) {
         setErrors("Source Method Name is required but is blank.");
         return;
      }

      if (isFileProvider() && !isFileNameValid())  {
         setErrors("Unsupported File Extension in Source Name.");
         return;
      }
      ...
      if (isSpreadSheet() && isBlank(getItemsContainerName())) {
         setErrors("Spread Sheet Name not provided for spreadsheet source.");
         return;
      }
   }
