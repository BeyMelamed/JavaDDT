...
public class Verifier {
   ...
   public static Verifier verify(String ev, String av, String md, String opt, String cls, boolean stripWhiteSpace) {
      ...
      try {
         ...
         v.setErrors(trim(v.getErrors() + " " + e.getCause().toString()));
      }
      finally {
         return v;
      }
   ...
   public static Verifier getVerifier(TestItem testItem) {
      ...
      String md =  testItem.getDataProperties().get("comparemode");
      if (isBlank(md))
         md = "";
      if (isBlank(md))
      ...
      String opt =  testItem.getDataProperties().get("option");
      if (isBlank(opt))
         opt = "";

      // User typically specifies some expected value
      String ev =  testItem.getDataProperties().get("value");
      if (isBlank(ev))
         ev = "";
      ...
      String strip =  testItem.getDataProperties().get("stripWhiteSpace");
      if (isBlank(strip))
      ...
      switch (getComp().toLowerCase()) {
         case "isblank" :case "isnull" :case "isempty":case "blank":case "null":case "empty":
         {
            ...
            if (!getComp().toLowerCase().startsWith("is"))
               prefix = "Is";
            if (isBlank(getAv()))
               setComments("Successful Verification - Actual " + prefix + " " + Util.sq(getComp()));
            else
               setErrors("Failed Verification - Actual (" + Util.sq(getAv() + ") is NOT (but is expected to be) BLANK / EMPTY / NULL"));
         ...
         {
            if (isBlank(getAv()))
               setErrors("Actual is BLANK but is expected to be NOT BLANK / EMPTY / NULL");
            else
               setComments("Actual (" +Util.sq(getAv()) + ") is not blank / empty / null.");
         ...
         switch (getCls().toLowerCase()) {
            ...
            case "int" :case "integer" :
            {
               ...
               if (isBlank(verifier.getErrors()))
                  verifier.verify();
               setComments(verifier.getComments());
               setErrors(verifier.getErrors());
               break;
            }
            ...
            default: setErrors("Invalid object class specified: " +Util.sq(getCls()));
         } // Switch

      } //Try
      catch (Exception e) {
         setErrors("Verifier generated general exception " + e.getCause().toString());
      }
   } // verify
   ...
   public void verifyStrings() throws Exception {
      ...
      if (expected.toLowerCase().startsWith("%date")) {
         ...
         try {
            ...
            if (dateParser.hasException()) {
               setErrors(dateParser.getException().getMessage().toString());
               return;
            }
            ...
            setErrors("Exception in Date Parser: " + e.getCause().toString());
            return;
         }
      }
      ...
      try {
         ...
         switch (compareMode.toLowerCase()) {
            ...
            case "equals" : case "equal" : case "is" : case "isenabled" :case "isdisplayed" :case "isselected" : // NOTE: booleans are represented as string!
            {
               if (actual.equals(expected))
                  setComments(getStandardComment());
               else
                  setErrors(getStandardError());
               break;
            }
            ...
            case "between": {
               ...
               if (getValues().length == 2) {
                  ...
                  setErrors("Invalid 'Between' String specifications.");
               break;
            }
            default: setErrors("Invalid comparison mode specified: "+Util.sq(getComp()));
         } // Switch
      } // Try
      catch (Exception e) {
         setErrors("Verifier generated general exception " + e.getCause().toString());
   ...
   private class NumberVerifier extends Verifier {
      ...
      public void verify(Number expected, Number actual) throws Exception {
         ...
         try {
            ...
            switch (compareMode) {
               case "equals" :case "equal" : case "is" :case "=" :case "==" :
               {
                  if (actual.equals(expected))
                     setComments(getStandardComment());
                  else
               ...
               case "between": {
                  ...
                  if (getValues().length == 2) {
                     ...
                     if (isBlank(blurb)) {
                        ...
                        setErrors("Invalid numeric specifications - " + blurb);
                  }
                  else
                     setErrors("Invalid 'Between' specifications.");
                  break;
   ...
   private class IntegerVerifier extends NumberVerifier {
      ...
      private void setValuesFromStrings() {
         ...
         if (!isBlank(blurb)) {
            setErrors(blurb + "Verification aborted.");
         }
      }
   ...
   private class DateVerifier extends Verifier {
      ...
      public void verify() throws Exception {
         ...
         try {
            ...
            switch (compareMode) {
               // all booleans will be evaluated against the strings true/false
               case "equals" :case "is" :case "equal" :
               {
               ...
               case "between" :
               {
                  ...
                  if (getValues().length == 2) {
                     ...
                     setErrors("Invalid 'Between' dates specifications.");
                  break;
               }
               default: setErrors("Invalid comparison mode specified: "+Util.sq(getComp()));
            } // Switch
